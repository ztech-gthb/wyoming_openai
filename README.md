...
| `--tts-cooldown-buffer-ms`              | `TTS_COOLDOWN_BUFFER_MS`                   | None (disabled)                               | Extra milliseconds added on top of the auto-calculated TTS audio duration to suppress STT after stream_end (total cooldown = audio duration + this value). Prevents echo loop in continue_conversation on fast networks where all audio bytes are transmitted before playback finishes. Must be a positive integer, max 60000. |
...
> [!WARNING]
> **`TTS_COOLDOWN_BUFFER_MS` uses a process-wide shared state.** The cooldown window is applied globally across all incoming connections. If two voice satellites issue concurrent requests, a TTS response from one satellite may temporarily suppress STT from the other. This is a known limitation: the Wyoming protocol carries no session or satellite identifier, so per-satellite scoping is not possible at this layer. In practice the impact is minimal unless satellites frequently speak simultaneously. A correct solution would require echo suppression at the satellite or Home Assistant pipeline level, where playback completion is observable.
